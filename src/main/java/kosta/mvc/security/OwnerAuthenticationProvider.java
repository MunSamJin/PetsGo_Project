...
public class OwnerAuthenticationProvider implements AuthenticationProvider {
	...
	public Authentication authenticate(Authentication auth) throws AuthenticationException {
		...
		String campRegNo = auth.getName();
		System.out.println("provider에서 campRegNo = " + campRegNo);
		
		Camp camp = campRep.selectCamp(campRegNo);
		System.out.println("rep 다녀온 provider member = " + camp.getCampRegNo());

		if(camp == null){
			throw new UsernameNotFoundException(camp + "는 등록되지 않은 사업자입니다."); //spring exception
		}
